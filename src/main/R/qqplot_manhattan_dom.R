...
qq.plot <- function(tab,lambda=T,stat,BA=F,plot=T,mod,
    ...
    #Calculating LAMBDA
    chi_list <- qchisq(na.omit(tab$frequentist_dom_pvalue),df=1,lower.tail=F);
    l.fac <- median(chi_list)/0.456;
    print(paste(c("LAMBDA = ",l.fac),collapse=""));

    #Setting format for plot
    pvals <- -log10((na.omit(tab$frequentist_dom_pvalue)));
    tab$col <- "ivory3"
    tab$col[tab$frequentist_dom_pvalue <= 5e-8] <- "deeppink"
    col <- tab$col[!is.na(tab$frequentist_dom_pvalue)]
    col <- col[order(pvals)]
...
library(sfsmisc)
    ...
    tab_file_data <- tab_file_data[tab_file_data$frequentist_dom_pvalue!=0,]

    p <- tab_file_data$frequentist_dom_pvalue
    p <- p[!is.na(p)]
    n <- length(p)
...
#   makeqqplot
    ...
    tab_man <- data.frame(SNP=tab_file_data$SNP,
                          ...
                          pvalue=tab_file_data$frequentist_dom_pvalue)
